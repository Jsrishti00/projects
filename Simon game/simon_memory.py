...
time = 500 #How much time we will wait between the game
score = 0
game_sequence = [] #keep track of Order of button sequence of game which is to be guessed
player_sequence = [] #keep the track of Order of button sequence player has pressed

#Define functions 
def pick_sequence():
    """Print the next value in sequence. Do not allow repeated values"""
    while True:
        value = random.randint(1,4)
        #Sequence is size 0,so take value regardless
    ...
    #Now the value is added to the sequence, play the ssequence
    play_sequence()
...
def play_sequence():
    ...
    #without delay , all button will animate at the same time.
    #The delay adds the 'time' variable to each .after()
    delay = 0
...
def animate(buton):
    ...
    buton.config(state = ACTIVE)
    root.after(time, lambda:buton.config(state= NORMAL))
...
def set_difficulty():
    """ Use radio buttons to set difficulty . Difficulty affects time between bytton flashes """
    global time
...
def check_round():
    ...
    #The player is correct , so change the label , update score , wait , then start next round
    if player_sequence == game_sequence:
        change_label('Correct!')
        score += len(player_sequence) + int(1000/time)
        root.after(500, pick_sequence)
    #The player is incorrect change label update score , disable button and rest for new game
    else:
        change_label('Wrong!')
        score = 0
        disable()
...
def disable():
    """ Disable all buttons as they can't accidently be passed """
...
#active background is when the particular is clicked
...
difficult_label =tkinter.Label(game_frame, text='Difficulty:', font=game_font2,bg=game_color)

easy_level= tkinter.Radiobutton(game_frame, text="Easy", variable=difficulty, value="Easy",font=game_font2, bg=game_color, command=set_difficulty)
medium_level = tkinter.Radiobutton(game_frame, text="Medium", variable=difficulty, value="Medium",font=game_font2, bg=game_color, command=set_difficulty)
hard_level= tkinter.Radiobutton(game_frame, text="Hard", variable=difficulty, value="Hard",font=game_font2, bg=game_color, command=set_difficulty)

difficult_label.grid(row=2,column=0)
easy_level.grid(row=2,column=1)
medium_level.grid(row=2,column=2)
hard_level.grid(row=2,column=3)
