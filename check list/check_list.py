...
def remove_item():
    ...
    # Anchor gives the index of the selected element 

def clear_list():
    """Delete all items of listbx"""
...
def save_list():
    ...
    with open('checklist.txt', 'w') as f:
        ...
        for item in list_tuple:
            #Take proper precaution to include only one new line
            if(item.endswith('\n')):
                f.write(item)
            else :
                f.write(item + "\n")
...
def open_list():
    """Open list in starting of program if there is one"""
    try:
